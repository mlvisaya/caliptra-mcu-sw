...
impl BootFlow for ColdBoot {
    fn run(env: &mut RomEnv, params: RomParameters) -> ! {
        ...
        let network_boot = params.request_network_boot;
        ...
        if network_boot {
            ...
            let image_provider =
                crate::recovery::network::NetworkImageProvider::new(&driver);
            driver.set_client(&image_provider);
            image_provider.ensure_initiated().unwrap_or_else(|_| fatal_error(McuError::ROM_COLD_BOOT_NETWORK_INITIATE_ERROR));
