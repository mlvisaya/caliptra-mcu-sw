...
mod test {
    ...
    use flash_image::{
        FlashHeader, ImageHeader, MCU_RT_IDENTIFIER, HEADER_VERSION, TFTP_IMAGE_MAGIC_NUMBER,
    };
    ...
    fn build_toc_image() -> Vec<u8> {
        ...
        let dummy_data: Vec<u8> = (0..TEST_IMAGE_SIZE as usize).map(|i| (i & 0xFF) as u8).collect();
        ...
        let hdr_bytes_for_cksum = &img_hdr.as_bytes()
            [..core::mem::offset_of!(ImageHeader, image_header_checksum)];
        ...
        let fh_bytes_for_cksum = &flash_hdr.as_bytes()
            [..core::mem::offset_of!(FlashHeader, header_checksum)];
