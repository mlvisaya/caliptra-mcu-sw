...
mod test {
    ...
    use mcu_builder::flash_image::{generate_image_info, FlashImage, FirmwareImage};
    ...
    fn write_tftp_content(
        ...
        for (fname, identifier, data) in files {
            ...
            std::fs::write(tftp_dir.join(fname), data)
                .expect("Failed to write firmware file");
        }

        let image_info = generate_image_info(images.clone());
        let flash_image = FlashImage::new(&images, &image_info, TFTP_IMAGE_MAGIC_NUMBER);
        std::fs::write(tftp_dir.join(TFTP_BOOT_FILENAME), flash_image.to_toc_bytes())
    ...
    fn test_network_boot() {
        ...
        assert!(!timed_out, "Test timed out after {:?} (checkpoint={})", TIMEOUT, checkpoint);
